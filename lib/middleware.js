...
var parseLocation = function parseLocation() {
	return location.hash && /\?/.test(location.hash) ? {
		...
		return function (next) {
			return function (action) {
				if (types.indexOf(action.type) !== -1) {
					var _parseLocation = parseLocation(),
					    prefix = _parseLocation.prefix,
					    suffix = _parseLocation.suffix,
					    search = _parseLocation.search;
					...
					var nextParams = include.reduce(function (params, key) {
						return _extends({}, params, _defineProperty({}, key, queryparamMiddlewareTransformer[key](action[key])));
					}, params);
					...
					history.replaceState(null, '', prefix + '?' + (0, _queryString.stringify)(nextParams) + suffix);
				}
