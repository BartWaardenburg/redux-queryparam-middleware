...
var createQueryparamMiddleware = exports.createQueryparamMiddleware = function createQueryparamMiddleware(_ref) {
	...
	var omit = _ref$omit === undefined ? [] : _ref$omit;
	var _ref$transformer = _ref.transformer;
	var transformer = _ref$transformer === undefined ? {} : _ref$transformer;
	return function () {
		return function (next) {
			return function (action) {
				if (types.indexOf(action.type) !== -1) {
					(function () {
						var params = (0, _queryString.parse)(location.search);
						...
						history.replaceState(null, '', location.pathname + '?' + (0, _queryString.stringify)(nextParams));
					})();
				}
